...
public:
    ...
    TimeInterval(const TimeOfDay &start, int length, const string &units);

    TimeInterval(const TimeInterval& start, int length);

    TimeOfDay GetStartTime() const { return startTime; }
    TimeOfDay GetEndTime() const { return endTime; }

    void SetStartTime(const TimeOfDay& start);
    void SetEndTime(const TimeOfDay& end);
    void SetEndTime(int length, const string& unit);
    ...
    bool isLaterInDay(const TimeOfDay& t1, const TimeOfDay& t2) const;

    TimeOfDay calculateEndTime(const TimeOfDay& start, int length, const string& units) const;
};
